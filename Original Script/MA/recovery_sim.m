function recovery_sim(launcher,orbit,site,data)
...
% %% Ascent second stage
% launcher.st02 = launcher.st2;
% launcher.st02.T_time = [0 launcher.st2.tb];
% launcher.st02.T = [0 0];
% Y02 = obj.Y(obj.T==(launcher.st2.tb+launcher.st1.tb),:)';
% Y02 = Y02(:,1);
% Y02(5) = launcher.st2.ms;
% Y02(6) = 0;
% chi = 0;
% [T2a, Y2a] = ode45(@rocket_dyn, [0 2000], Y02, options, launcher.st02, site, orbit, chi);

%% Ascent Third stage
% launcher.st03 = launcher.st3;
% launcher.st03.T_time = [0 launcher.st3.tb];
% launcher.st03.T = [0 0];
% Y03 = obj.Y(end,:);
% Y03(5) = launcher.st3.ms;
% Y03(6) = 0;
% chi = 0;
% [T3a, Y3a] = ode45(@rocket_dyn, [0 7500], Y03, options, launcher.st03, site, orbit, chi);
% 
% %% Descent 
t0 = T1a(end);
...
%xnorth2 = cosd(data.site.azimuth).*Y2a(:,1);
%yeast2 = sind(data.site.azimuth).*Y2a(:,1);
% xnorth3 = cosd(data.site.azimuth).*Y3a(:,1);
% yeast3 = sind(data.site.azimuth).*Y3a(:,1);
...
% [X,Y,Z] = ned2ecef(xnorth,yeast,-obj.Y(:,2),data.site.lat0, data.site.lon0,-data.site.z0,wgs84Ellipsoid);
% X = X/1000;
% Y = Y/1000;
% Z = Z/1000;
% N = cross([X(end) Y(end) Z(end)],[X(end-1) Y(end-1) Z(end-1)]);
% n = N/norm(N);
% R_vect = [X(end) Y(end) Z(end)];
% r = R_vect ./ norm([X(end) Y(end) Z(end)]);
% v = cross(r,n);
% V_vect = obj.Y(end,3).*v ./ 1000;
% orbFig = figure();
% plotPlanet(3,[0 0 0],orbFig);
% hold on 
% plot3(X,Y,Z)
% RR=ode_solved(R_vect,V_vect,orbit.mu,6653,1,0,true);
...
%plotOrbit(6378+400,0,0,0,omega(end),pi,1.2*pi,0.01,orbit.mu,'r','2D',false)
xlabel('X [km]')
ylabel('Y [km]')
xlim([-8e3 8e3])
ylim([-8e3 8e3])
