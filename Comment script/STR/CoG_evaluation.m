...
% first evaluate the CoG considering each stage both full and empty and interpolating linearly
% then evaluate the CoG for each second of burning considering a mean mass flow rate for each second
% for the third stage for semplicity consider half of the structural mass in the cylinder and half in the cone

g0 = data.orbit.g;  %gravity acceleration [m/s^2]

% mass of each stage [kg]
m1 = launcher.st1.mp + launcher.st1.ms; 
m2 = launcher.st2.mp + launcher.st2.ms;
m3 = launcher.st3.mp + launcher.st3.ms;

m3_s = launcher.st3.ms; % stage 3 structural mass [kg]
mpay = launcher.st3.pay; % payload mass [kg]

TW3_ratio = launcher.st3.T(1) / ((m3-m3_s/2)*g0); % Thrust over weight ratio for third stage

% Specific Impulse of each stage [s]
...
% X_cg of each stage computed from the tip of the nose [m]
x_cg1_stat = Ltot - L1/2;
...
% X_cg of the entire rocket linearly interpolated [m]
...
tb_vect = [0,tb1,tb1,tb1+tb2,tb1+tb2,tb1+tb2+tb3];  %vector of each stage separation step [s]
...
% vector of x_cg for each second [m]
